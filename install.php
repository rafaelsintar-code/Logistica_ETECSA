...
$statements = [
    ...
    // ── CAMBIO LDAP: password_hash es NULL para usuarios de AD ──────────
    // auth_source distingue 'local' (contraseña en la app) vs 'ldap' (AD)
    ...
    // --- COLUMNAS ADICIONALES (idempotentes: seguras si ya existen) ---
    ...
    // ── COLUMNAS LDAP (idempotentes) ─────────────────────────────────────
    // Permite ejecutar install.php sobre una BD existente sin romper datos.
    "ALTER TABLE usuario ALTER COLUMN password_hash DROP NOT NULL",
    "ALTER TABLE usuario ADD COLUMN IF NOT EXISTS auth_source VARCHAR(10) NOT NULL DEFAULT 'local' CHECK (auth_source IN ('local', 'ldap'))",

    // Marcar usuarios pre-existentes como locales si aún no tienen auth_source
    "UPDATE usuario SET auth_source = 'local' WHERE auth_source IS NULL OR auth_source = ''",

    // Índice de búsqueda por username
