document.addEventListener("DOMContentLoaded", () => {
  ...
  async function cargarTablaMB51() {
    ...
    try {
      const res  = await fetch(`../../api/mb51/get_mb51.php?${params}`);
  ...
  function renderizarTabla(filas) {
    ...
    if (filas.length === 0) {
      tbody.innerHTML = `<tr><td colspan="7" style="text-align:center;">No hay datos cargados</td></tr>`;
    } else {
      ...
      filas.forEach(row => {
        ...
        tr.innerHTML = `
          ...
          <td>${escapeHtml(String(row.cantidad))}</td>
          <td>${escapeHtml(String(row.fecha_doc))}</td>
          <td>${escapeHtml(padCodigo(row.almacen))}</td>
          <td>${escapeHtml(row.clase_mov)}</td>
          <td>${escapeHtml(row.desc_clase_mov)}</td>
        `;
