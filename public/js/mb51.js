document.addEventListener("DOMContentLoaded", () => {
  ...
  async function cargarTablaMB51() {
    try {
      ...
      renderizarTabla(datosMB51);

    } catch (e) {
      console.error("Error cargando MB51:", e);
  ...
  function renderizarTabla(lista) {
    ...
    if (!lista.length) {
      tbody.innerHTML = `
        <tr>
          <td colspan="8" style="text-align:center;">No hay datos cargados</td>
        </tr>`;
      return;
    }
  ...
  function filtrar() {
    ...
    const resultado = datosMB51.filter(row =>
