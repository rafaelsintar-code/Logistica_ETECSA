...
const seleccionPorId    = new Map();   // Map<id, Map<codigo, {articulo, cantidad}>>
let filaActiva          = null;
...
async function cargarSolicitudes() {
  // Limpiar expiradas
  try {
    const resL  = await fetch("../../api/solicitud/limpiar_solicitudes.php", { method: "POST" });
    const dataL = await resL.json();
    if (dataL.success && dataL.eliminadas.length > 0) {
      dataL.eliminadas.forEach(id => seleccionPorId.delete(id));
      mostrarToast(`${dataL.eliminadas.length} solicitud(es) expirada(s) eliminada(s).`, "info");
    }
  } catch (e) {}
  ...
  try {
    ...
    const data = await res.json();
    if (!Array.isArray(data)) { mostrarToast("Error cargando solicitudes.", "error"); return; }
    ...
    // Para cada solicitud, inicializar mapa y cargar artículos guardados en BD
    await Promise.all(data.map(async s => {
      if (!seleccionPorId.has(s.id)) seleccionPorId.set(s.id, new Map());
      // Solo cargamos artículos de solicitudes no confirmadas (las confirmadas
      // solo se ven en preview/documento)
      if (!s.hora_confirmacion) {
        await cargarArticulosSolicitudBD(s.id);
      }
    }));

    renderizarTabla();
  } catch (e) { mostrarToast("Error cargando solicitudes.", "error"); }
}

/* ─── CARGAR ARTÍCULOS DE UNA SOLICITUD DESDE BD ── */
async function cargarArticulosSolicitudBD(id) {
  try {
    const res  = await fetch(`../../api/solicitud/get_articulos_solicitud.php?id=${id}`);
    const data = await res.json();
    ...
    data.forEach(art => {
      mapa.set(String(art.codigo), {
        ...
        articulo: { codigo_articulo: art.codigo, descripcion: art.descripcion, familia: "" }
      });
    });
...
function renderizarTabla() {
  ...
  if (filtradas.length === 0) {
    tbody.innerHTML = `<tr id="sol-empty-row"><td colspan="9" style="text-align:center;">Sin solicitudes para este período</td></tr>`;
    return;
  }
  ...
  filtradas.forEach(s => {
    ...
    const seleccion   = seleccionPorId.get(s.id);
    ...
    if (confirmada) {
      ...
      colConfirmar = `<button class="sol-btn sol-btn-confirmar" onclick="confirmarSolicitud(${s.id})">
    ...
    if (!confirmada) {
      ...
      colArticulos = `<button class="sol-btn sol-btn-agregar" onclick="abrirModalArticulos(${s.id})">
    ...
    let colAcciones = "";
    if (!confirmada) {
      colAcciones = `<button class="sol-btn sol-btn-doc" onclick="abrirPreview(${s.id})" title="Ver documento">&#128196;</button>
                     <button class="sol-btn sol-btn-descartar" title="Descartar" onclick="descartarSolicitud(${s.id})">✕</button>`;
    }
...
async function agregarSolicitud() {
  ...
  try {
    const res  = await fetch("../../api/solicitud/add_solicitud.php", {
      ...
      body:    JSON.stringify({ codigo_almacen: codigo, tipo_almacen: opt.dataset.tipo, nombre_pv: opt.dataset.nombre })
    });
    const resp = await res.json();
    if (resp.success === false) { mostrarToast(resp.message || "Error al crear solicitud.", "error"); return; }

    solicitudesCargadas.unshift({
      id: resp.id, codigo_almacen: codigo, tipo_almacen: opt.dataset.tipo,
      nombre_pv: opt.dataset.nombre, usuario: resp.usuario,
      hora_creacion: resp.hora_creacion, hora_confirmacion: null, vale: null
    });
    seleccionPorId.set(resp.id, new Map());
    renderizarTabla();
    mostrarToast("Solicitud creada.", "success");
  } catch (e) { mostrarToast("Error al crear solicitud.", "error"); }
...
function descartarSolicitud(id) {
  mostrarConfirmacion("¿Descartar esta solicitud?", async () => {
    ...
    } catch (e) { mostrarToast("Error al eliminar solicitud.", "error"); }
  });
}
  ...
  mostrarConfirmacion("¿Confirmar esta solicitud?", async () => {
    ...
    try {
      ...
      if (resp.success === false) {
        mostrarToast(resp.message || "Error al confirmar.", "error");
...
async function abrirPreview(id) {
  ...
  // Cargar artículos frescos desde BD para el preview
  let articulos = [];
  try {
    const res  = await fetch(`../../api/solicitud/get_articulos_solicitud.php?id=${id}`);
    const data = await res.json();
...
async function cargarArticulos() {
  ...
  try {
    ...
    const data = await res.json();
    if (!Array.isArray(data)) { mostrarToast("Error cargando artículos.", "error"); return; }
    articulosCargados = data;
  } catch (e) { mostrarToast("Error cargando artículos.", "error"); }
}
...
async function abrirModalArticulos(id) {
  ...
  if (articulosCargados.length === 0) { mostrarToast("No hay artículos disponibles.", "error"); return; }

  // Restaurar selección temporal desde el mapa en memoria (ya cargado desde BD)
  seleccionTemporal = new Map();
  const seleccion   = seleccionPorId.get(id);
  if (seleccion) seleccion.forEach((val, key) => seleccionTemporal.set(key, val.cantidad));
...
async function confirmarSeleccionArticulos() {
  ...
  // Construir lista de artículos con descripción
  ...
  seleccionTemporal.forEach((cantidad, clave) => {
    ...
    if (articulo) articulosPayload.push({ codigo: clave, descripcion: articulo.descripcion, cantidad });
  });

  // Guardar en BD
  try {
    const res  = await fetch("../../api/solicitud/add_articulos_solicitud.php", {
      method:  "POST",
      headers: { "Content-Type": "application/json" },
      body:    JSON.stringify({ solicitud_id: filaActiva, articulos: articulosPayload })
    });
    const resp = await res.json();
