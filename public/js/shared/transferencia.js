...
/* ─── CARGAR SELECTS DE ALMACENES ───────────────── */
async function cargarAlmacenesSelects() {
  try {
    const res  = await fetch("../../api/solicitud/get_almacenes_solicitud.php");
    const data = await res.json();
    if (!Array.isArray(data)) { mostrarToast("Error cargando almacenes.", "error"); return; }
    almacenesDisponibles = data;
    poblarSelectOrigen();
  } catch (e) { mostrarToast("Error cargando almacenes.", "error"); }
}
...
function onOrigenChange() {
  ...
  if (val === "") {
    document.getElementById("trf-select-destino").innerHTML = '<option value="">-- Almacén destino --</option>';
...
async function cargarTransferencias() {
  try {
    const resL  = await fetch("../../api/transferencia/limpiar_transferencias.php", { method: "POST" });
    const dataL = await resL.json();
    if (dataL.success && dataL.eliminadas.length > 0) {
      dataL.eliminadas.forEach(id => seleccionPorId.delete(id));
      mostrarToast(`${dataL.eliminadas.length} transferencia(s) expirada(s) eliminada(s).`, "info");
    }
  } catch (e) {}
  ...
  try {
    ...
    const data = await res.json();
    if (!Array.isArray(data)) { mostrarToast("Error cargando transferencias.", "error"); return; }

    _esAdmin  = (res.headers.get("X-Usuario-Rol") || "") === "administrador";
    _username = res.headers.get("X-Usuario-Username") || "";

    transferenciasGuardadas = data;

    // Cargar artículos de BD para cada transferencia no confirmada
    await Promise.all(data.map(async t => {
      if (!seleccionPorId.has(t.id)) seleccionPorId.set(t.id, new Map());
      if (!t.hora_confirmacion) {
        await cargarArticulosTransferenciaBD(t.id);
      }
    }));

    renderizarTabla();
  } catch (e) { mostrarToast("Error cargando transferencias.", "error"); }
}

/* ─── CARGAR ARTÍCULOS DE UNA TRANSFERENCIA DE BD ─ */
async function cargarArticulosTransferenciaBD(id) {
  try {
    const res  = await fetch(`../../api/transferencia/get_articulos_transferencia.php?id=${id}`);
    const data = await res.json();
    if (!Array.isArray(data)) return;

    const mapa = seleccionPorId.get(id) || new Map();
    data.forEach(art => {
      mapa.set(String(art.codigo), {
        cantidad: art.cantidad,
        articulo: { codigo_articulo: art.codigo, desc_articulo: art.descripcion, stock: 0 }
      });
    });
    seleccionPorId.set(id, mapa);
  } catch (e) {}
}
...
function renderizarTabla() {
  ...
  if (filtradas.length === 0) {
    tbody.innerHTML = `<tr><td colspan="9" style="text-align:center;">Sin transferencias para este período</td></tr>`;
    return;
  }
  ...
  filtradas.forEach(t => {
    ...
    const seleccion   = seleccionPorId.get(t.id);
    const numArts     = seleccion ? seleccion.size : 0;
    ...
    let colConfirmar;
    if (confirmada) {
      colConfirmar = `<span style="color:#27ae60; font-weight:600;">${fechaConf}</span>`;
    } else if (_esAdmin) {
      colConfirmar = `<button class="sol-btn sol-btn-confirmar" onclick="confirmarTransferencia(${t.id})">
                        <span class="sol-btn-icon">✓</span> Confirmar
                      </button>`;
    } else {
      colConfirmar = `<span style="color:#888; font-style:italic;">Pendiente</span>`;
    }

    let colArticulos;
    if (!confirmada) {
      // Cualquier usuario puede agregar/editar artículos mientras no esté confirmada
      colArticulos = `<button class="sol-btn sol-btn-agregar" onclick="abrirModalArticulos(${t.id})">
                        <span class="sol-btn-icon">＋</span> Artículos${numArts > 0 ? ` (${numArts})` : ""}
                      </button>`;
    } else if (numArts > 0) {
      colArticulos = `<span style="color:#555;">${numArts} artículo(s)</span>`;
    } else {
      colArticulos = `—`;
    }
    ...
    let colAcciones = "";
    if (!confirmada) {
      colAcciones = `<button class="sol-btn sol-btn-doc" onclick="abrirPreview(${t.id})" title="Ver documento">&#128196;</button>
                     <button class="sol-btn sol-btn-descartar" title="Descartar" onclick="descartarTransferencia(${t.id})">✕</button>`;
    }

    tr.innerHTML = `
      <td>${escapeHtml(String(t.almacen_origen))}</td>
      <td>${escapeHtml(t.denom_origen)}</td>
      <td>${escapeHtml(String(t.almacen_destino))}</td>
      <td>${escapeHtml(t.denom_destino)}</td>
      ...
      <td style="text-align:center;">${colConfirmar}</td>
      <td style="text-align:center;">${colArticulos}</td>
      <td style="text-align:center; white-space:nowrap;">${colAcciones}</td>
    `;
...
async function agregarTransferencia() {
  ...
  try {
    const res  = await fetch("../../api/transferencia/add_transferencia.php", {
      ...
      body: JSON.stringify({
        almacen_origen: origen, denom_origen: optOrigen.dataset.denom,
        almacen_destino: destino, denom_destino: optDestino.dataset.denom
      })
    });
    const resp = await res.json();
    if (resp.success === false) { mostrarToast(resp.message || "Error al crear transferencia.", "error"); return; }

    transferenciasGuardadas.unshift({
      id: resp.id, almacen_origen: origen, denom_origen: optOrigen.dataset.denom,
      almacen_destino: destino, denom_destino: optDestino.dataset.denom,
      usuario: resp.usuario, hora_creacion: resp.hora_creacion,
      hora_confirmacion: null, vale: null
    });
    seleccionPorId.set(resp.id, new Map());
    renderizarTabla();
    mostrarToast("Transferencia creada.", "success");
  } catch (e) { mostrarToast("Error al crear transferencia.", "error"); }
  finally { btnAgregar.disabled = false; }
}
...
function descartarTransferencia(id) {
  mostrarConfirmacion("¿Descartar esta transferencia?", async () => {
    try {
      ...
      const resp = await res.json();
      if (resp.success === false) { mostrarToast(resp.message || "Error al eliminar.", "error"); return; }
    ...
    } catch (e) { mostrarToast("Error al eliminar transferencia.", "error"); }
  });
}
...
function confirmarTransferencia(id) {
  ...
  mostrarConfirmacion("¿Confirmar esta transferencia? Se actualizará el stock de ambos almacenes.", async () => {
    ...
    try {
      // El backend ahora lee los artículos desde transferencia_articulo
...
async function abrirPreview(id) {
  ...
  // Cargar artículos frescos desde BD
  let articulos = [];
  try {
    const res  = await fetch(`../../api/transferencia/get_articulos_transferencia.php?id=${id}`);
    const data = await res.json();
...
async function abrirModalArticulos(id) {
  ...
  try {
    ...
    const data = await res.json();
    if (!Array.isArray(data)) { mostrarToast("Error cargando artículos del almacén.", "error"); return; }
    articulosOrigenCargados = data;
  } catch (e) { mostrarToast("Error cargando artículos del almacén.", "error"); return; }
  ...
  if (seleccionGuardada) seleccionGuardada.forEach((val, key) => seleccionTemporal.set(key, val.cantidad));
...
function renderizarTablaArticulos(lista) {
  ...
  lista.forEach(art => {
    ...
    chk.addEventListener("change", () => {
      ...
      if (chk.checked) { seleccionTemporal.set(clave, parseInt(inp.value) || 1); inp.focus(); }
      else              { seleccionTemporal.delete(clave); }
    });
...
/* ─── CONFIRMAR SELECCIÓN DEL MODAL — GUARDA EN BD ─ */
async function confirmarSeleccionArticulos() {
  if (!filaActiva) return;

  for (const [codigo, cantidad] of seleccionTemporal) {
    if (!cantidad || cantidad < 1) { mostrarToast("La cantidad debe ser mayor a 0.", "error"); return; }
    const art = articulosOrigenCargados.find(a => String(a.codigo_articulo) === codigo);
    if (art && cantidad > parseInt(art.stock)) {
      mostrarToast(`Cantidad supera el stock para "${art.desc_articulo}" (máx. ${art.stock}).`, "error");
      return;
    }
  }
  ...
  seleccionTemporal.forEach((cantidad, clave) => {
    ...
    if (articulo) articulosPayload.push({ codigo: clave, descripcion: articulo.desc_articulo, cantidad });
  });

  // Guardar en BD
  try {
    const res  = await fetch("../../api/transferencia/add_articulos_transferencia.php", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body:   JSON.stringify({ transferencia_id: filaActiva, articulos: articulosPayload })
    });
    const resp = await res.json();
    if (!resp.success) { mostrarToast("Error guardando artículos.", "error"); return; }
  } catch (e) { mostrarToast("Error guardando artículos.", "error"); return; }
  ...
  mostrarToast(
    total > 0 ? `${total} artículo(s) guardado(s).` : "No se seleccionó ningún artículo.",
