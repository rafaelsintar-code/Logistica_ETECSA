document.addEventListener("DOMContentLoaded", () => {
  ...
  // ACCIÓN RECOMENDADA
  // ==========================
  function obtenerAccionRecomendada(indicacion) {
  ...
  function renderTabla(filtrados) {
    ...
    if (pagina.length === 0) {
      tbody.innerHTML = `<tr><td colspan="7" style="text-align:center;">No hay resultados para los filtros seleccionados</td></tr>`;
    } else {
      pagina.forEach(row => {
        ...
        const indicacion = obtenerIndicacion(row);
        switch (indicacion) {
        ...
        const accion = obtenerAccionRecomendada(indicacion);

        tr.innerHTML = `
          <td>${escapeHtml(String(row.material))}</td>
          <td>${escapeHtml(row.desc_articulo)}</td>
          <td>${escapeHtml(String(row.almacen))}${row.nombre_pv ? ' — ' + escapeHtml(row.nombre_pv) : ''}</td>
