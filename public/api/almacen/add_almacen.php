...
try {
    ...
    // Verificar en una sola query que ningún código enviado ya exista
    $codigosNoNulos = array_filter($codigos);
    if (!empty($codigosNoNulos)) {
        $placeholders = implode(',', array_fill(0, count($codigosNoNulos), '?'));
        $dup = $pdo->prepare("SELECT codigo_almacen FROM cod_almacen WHERE codigo_almacen IN ($placeholders)");
        $dup->execute(array_values($codigosNoNulos));
        $existente = $dup->fetchColumn();
        if ($existente !== false) {
            $pdo->rollBack();
            $tipoEncontrado = array_search($existente, $codigosNoNulos);
            http_response_code(409);
            echo json_encode(["success" => false, "message" => "El código $existente ($tipoEncontrado) ya está en uso."]);
            exit;
        }
    }
