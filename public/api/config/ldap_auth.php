...
class LdapAuth
{
    ...
    public function __construct()
    {
        ...
        $this->port      = (int)($l['port'] ?? 389);
        $this->baseDn    = $l['base_dn']    ?? '';
        $this->bindDn    = $l['bind_dn']    ?? '';
        $this->bindPass  = $l['bind_pass']  ?? '';
        $this->loginAttr = $l['login_attr'] ?? 'sAMAccountName';
        $this->useTls    = filter_var($l['use_tls'] ?? false, FILTER_VALIDATE_BOOLEAN);
    }
