...
try {
    $tmpPath    = excel_validar_y_copiar('mb51_');
    ...
    $insert = $pdo->prepare('
        INSERT INTO mb51 (material, desc_articulo, cantidad, fecha_doc, almacen, clase_mov, desc_clase_mov)
        VALUES (:material, :desc_articulo, :cantidad, :fecha_doc, :almacen, :clase_mov, :desc_clase_mov)
    ');
    ...
    for ($startRow = 2; $startRow <= $totalFilas; $startRow += EXCEL_CHUNK_SIZE) {
        ...
        foreach ($spreadsheet->getActiveSheet()->getRowIterator($startRow, $startRow + EXCEL_CHUNK_SIZE - 1) as $row) {
            ...
            [$material, $desc_articulo, $cantidad, $fecha_doc_raw, $almacen, $clase_mov, $desc_clase_mov]
                = array_pad($cells, 7, '');
            $resumen = implode(' | ', [$material, $desc_articulo, $cantidad, $fecha_doc_raw, $almacen, $clase_mov, $desc_clase_mov]);

            if (!$material || !$desc_articulo || $cantidad === '' || !$fecha_doc_raw || !$almacen || !$clase_mov || !$desc_clase_mov) {
                $erroresExcel++;
            ...
            if (!ctype_digit($material) || strlen($material) !== 10) {
                $erroresExcel++;
                if (count($lineasError) < EXCEL_MAX_ERR_LINEAS) $lineasError[] = "[ERROR EXCEL - Material inválido] Fila $numFila\n  $resumen";
                continue;
            }
            ...
            if (!is_numeric($cantidad)) {
                $erroresExcel++;
                if (count($lineasError) < EXCEL_MAX_ERR_LINEAS) $lineasError[] = "[ERROR EXCEL - Cantidad no numérica] Fila $numFila\n  $resumen";
                continue;
            }
            $fecha_doc = excel_parse_fecha($fecha_doc_raw);
            if (!$fecha_doc) {
                $erroresExcel++;
                if (count($lineasError) < EXCEL_MAX_ERR_LINEAS) $lineasError[] = "[ERROR EXCEL - Fecha inválida] Fila $numFila\n  $resumen";
                continue;
            }
            ...
            $insert->execute([':material' => $material, ':desc_articulo' => $desc_articulo,
                ':cantidad' => (float)$cantidad, ':fecha_doc' => $fecha_doc,
                ':almacen' => $almacenPad, ':clase_mov' => $clase_mov, ':desc_clase_mov' => $desc_clase_mov]);
            $importados++;
        }
    ...
    if (!empty($lineasError)) {
        $cab = "REPORTE DE ERRORES - IMPORTACIÓN MB51\nFecha: " . date('d/m/Y H:i:s') . "\n"
