...
try {
    ...
    // Cargar artículos desde la tabla transferencia_articulo
    $stmtArts = $pdo->prepare('
        SELECT codigo_articulo AS codigo, cantidad
        FROM transferencia_articulo
        WHERE transferencia_id = :id
    ');
    $stmtArts->execute([':id' => $id]);
    $articulos = $stmtArts->fetchAll(PDO::FETCH_ASSOC);

    if (count($articulos) === 0) {
        $pdo->rollBack();
        echo json_encode(["success" => false, "message" => "Debe agregar al menos un artículo antes de confirmar."]);
        exit;
    }
