...
try {
    ...
    // Verificar que la transferencia existe, no está confirmada y (si no es admin) pertenece al usuario
    $sql = "SELECT id FROM transferencia WHERE id = :id AND hora_confirmacion IS NULL";
    if (!$esAdmin) $sql .= " AND usuario = :usuario";

    $check = $pdo->prepare($sql);
    $params = [':id' => $transferenciaId];
    ...
    if (!$check->fetch()) {
        ...
        echo json_encode(['success' => false, 'message' => 'Transferencia no encontrada, ya confirmada o sin permisos.']);
        exit;
    }
