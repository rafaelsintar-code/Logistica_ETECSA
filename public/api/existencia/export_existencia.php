...
try {
    ...
    function obtenerAccionRecomendada(string $indicacion): string {
        if ($indicacion === 'verde')                                   return 'Transferencia';
        if ($indicacion === 'rojo' || $indicacion === 'amarillo')      return 'Asignación';
        return ''; // azul (sin ventas) y gris (normal) → sin acción
    }
    ...
    // ---- Cabeceras ----
    $headers = ['Código Artículo', 'Descripción', 'Almacén', 'Cantidad', 'Promedio Ventas', 'Disponibilidad', 'Acción Recomendada'];
    $col     = 'A';
    ...
    foreach ($rows as $row) {
        ...
        $accion      = obtenerAccionRecomendada($indicacion);

        $sheet->setCellValue("A{$fila}", $row['material']);
        $sheet->setCellValue("B{$fila}", $row['desc_articulo']);
        $sheet->setCellValue("C{$fila}", $row['almacen'] . ($row['nombre_pv'] ? ' — ' . $row['nombre_pv'] : ''));
        ...
        $sheet->setCellValue("G{$fila}", $accion);
    ...
    // Si ob_start() ya capturó contenido parcial del xlsx, limpiarlo
    // antes de enviar la respuesta JSON de error para evitar cabeceras mixtas.
