...
try {
    ...
    // Verificar que la solicitud existe, no está confirmada y pertenece al usuario
    $check = $pdo->prepare("
        SELECT id FROM solicitud
        WHERE id = :id AND hora_confirmacion IS NULL
    ");
    $check->execute([':id' => $solicitudId]);
    if (!$check->fetch()) {
        $pdo->rollBack();
        http_response_code(404);
        echo json_encode(['success' => false, 'message' => 'Solicitud no encontrada o ya confirmada.']);
        exit;
    }
